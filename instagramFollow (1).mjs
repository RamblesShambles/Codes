...
async function main() {
  try {
    ...
    // Please put the Post link after the "--new-window" text in await exec.
    log('Opening Chrome and navigating to Reddit Post...');
    await exec('google-chrome --new-window https://www.instagram.com/onepiece_staff/');
    ...
    const screenshotNameInitial = `Follow_initial_${new Date().toISOString().replace(/:/g, '-')}.png`;
    await takeScreenshot(screenshotNameInitial);

    // Press Tab 14 times with random delay
    log('Pressing Tab 14 times to reach Follow button...');
    await pressTabRandomly(14);
    ...
    const screenshotNameFinal = `Follow_final_${new Date().toISOString().replace(/:/g, '-')}.png`;
    log(`Final screenshot name: ${screenshotNameFinal}`);
    await takeScreenshot(screenshotNameFinal);

    log('Automation successful. Followed successfully.');
    return JSON.stringify({
      success: true,
      message: 'Followed Successfully',
      data: {
        visitedUrl: 'https://www.instagram.com/onepiece_staff/',
