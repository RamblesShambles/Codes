...
async function main() {
  try {
    ...
    await exec('google-chrome --new-window https://www.reddit.com/r/memes/comments/1dmjbnm/americans_are_weirdos/');
    ...
    // Press Tab 18 times with random delay
    log('Pressing Tab 18 times to reach upvote button...');
    await pressTabRandomly(17);
    ...
    const screenshotNameFinal = `Downvote_final_${new Date().toISOString().replace(/:/g, '-')}.png`;
    log(`Final screenshot name: ${screenshotNameFinal}`);
    await takeScreenshot(screenshotNameFinal);

    log('Automation successful. Downvoted successfully.');
    return JSON.stringify({
      success: true,
      message: 'Downvoted Successfully',
      data: {
        visitedUrl: 'https://www.reddit.com/r/memes/comments/1dmjbnm/americans_are_weirdos/',
